...
class InitControllerMEnv(AbstractBaseController):
    class Meta:
        label = 'menvinit'
        description = strings['menvinit.info']
        arguments = [
            (['application_name'], dict(
                help=flag_text['menvinit.name'], nargs='?', default=[])),
            (['-r1', '--region1'], dict(help=flag_text['menvinit.region1'])),
            (['-r2', '--region2'], dict(help=flag_text['menvinit.region2'])),
            (['-p', '--platform'], dict(help=flag_text['menvinit.platform'])),
            (['-k', '--keyname'], dict(help=flag_text['menvinit.keyname'])),
            (['-k2', '--keyname2'], dict(help=flag_text['menvinit.keyname2'])),
            (['-i', '--interactive'], dict(
                action='store_true', help=flag_text['menvinit.interactive'])),
        ]
        usage = 'eb init <application_name> [options ...]'
        epilog = strings['menvinit.epilog']
	print "heremenv"
    def do_command(self):
        # get arguments
        self.interactive = self.app.pargs.interactive
        self.region1 = self.app.pargs.region1
        self.region2 = self.app.pargs.region2
        ...
        if self.interactive:
            self.region1 = self.get_region()
            self.region2 = self.get_region()
        else:
            self.region1 = self.app.pargs.region1
            self.region2 = self.app.pargs.region2
        #aws.set_region(self.region)
	print "inside commands"
        ...
        initializeops.menv_setup(self.app_name, self.region1, self.region2, self.solution)
    ...
    def get_region_from_inputs(self):
        ...
        region1 = self.app.pargs.region1
